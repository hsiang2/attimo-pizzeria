...
import { useProductById, useProductsByCategory } from '../react-query';
...
const ProductPage = () => {
    ...
    const { data: productsData, productsIsLoading } = useProductsByCategory(product.category || '')
                    ...
                    {!!cartId ? 
                        ...
                        <div className='container' style={{margin: '4.75rem auto 9rem'}}>
                            <div style={{display: 'flex', width: '6.4rem', marginLeft: 'auto', marginRight: 'auto', justifyContent: 'space-between', alignItems: 'center'}}>
                                <Link to={ productIndex - 1 >= 0 ? `/menu/id/${allProducts[productIndex - 1].id}` : '#'}>
                                ...
                                <Link to={ productIndex + 1 < allProducts.length ? `/menu/id/${allProducts[productIndex + 1].id}` : '#'}>
